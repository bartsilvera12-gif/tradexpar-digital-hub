-- -----------------------------------------------------------------------------
-- Envío por ciudad: el checkout ahora calcula precio y plazo según la ciudad
-- elegida (tabla CITY_SHIPPING_TARIFFS en el frontend) y envía el importe y la
-- etiqueta directamente. Reemplaza p_shipping_option por p_shipping_fee y
-- p_shipping_label, drop de la versión anterior (firma 17 params).
-- -----------------------------------------------------------------------------

-- 1) Eliminar la versión anterior (con p_shipping_option) — firma exacta.
drop function if exists tradexpar.create_checkout_order(
  text, text, text, text, text, uuid, text, jsonb, text, text, text, text, text, text, text, text, text
);

-- 2) Nueva versión: recibe p_shipping_fee (numérico) y p_shipping_label (texto)
--    en vez de la opción categórica. Mantiene toda la lógica previa
--    (revalidación de stock, cálculo de total, afiliados).
...
begin
  ...
  v_ship := round(greatest(0, coalesce(p_shipping_fee, 0))::numeric, 2);
  v_ship_label := coalesce(nullif(trim(p_shipping_label), ''), 'Envío');
...
grant execute on function tradexpar.create_checkout_order(
  text, text, text, text, text, uuid, text, jsonb, text, text, text, text, text, text, numeric, text, text, text
