-- =============================================================================
-- MANUAL (Supabase SQL Editor): columnas ciudad Dropi + RPC create_checkout_order
-- No ejecutar desde la herramienta del repo; copiar y pegar por bloques si preferís.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- BLOQUE 1 — Columnas en tradexpar.orders
-- -----------------------------------------------------------------------------
alter table tradexpar.orders
  add column if not exists customer_city_name text,
  add column if not exists customer_dropi_city_code text;

comment on column tradexpar.orders.customer_city_code is
  'Código de ciudad del checkout / PagoPar (ej. paraguay_cities.pagopar_city_code).';
comment on column tradexpar.orders.customer_city_name is
  'Nombre visible de ciudad (etiqueta checkout); útil para mapeos por nombre hacia Dropi.';
comment on column tradexpar.orders.customer_dropi_city_code is
  'Si está definido, el servidor usa este código al crear el pedido en Dropi (override manual).';


-- -----------------------------------------------------------------------------
-- BLOQUE 2 — (Opcional) columna en paraguay_cities para futuro mapeo en SQL/UI
-- -----------------------------------------------------------------------------
-- alter table tradexpar.paraguay_cities
--   add column if not exists dropi_city_code text;
-- comment on column tradexpar.paraguay_cities.dropi_city_code is
--   'Código sucursal/ciudad válido en Dropi; si null, aplica DROPI_CITY_MAP_JSON en el server.';


-- -----------------------------------------------------------------------------
-- BLOQUE 3 — Reemplazar tradexpar.create_checkout_order (firma 17 parámetros)
-- -----------------------------------------------------------------------------
drop function if exists tradexpar.create_checkout_order(text, text, text, text, text, uuid, text, jsonb, text, text, text, text, text, text, text, text, text);
